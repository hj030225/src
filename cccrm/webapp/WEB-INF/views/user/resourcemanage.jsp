		...
		<jsp:include page="/WEB-INF/views/layout/head.jsp" />
			<div class="bottom">
				...
				<table width="700px" >
					...
					<c:forEach var="programmer" items="${programmer}">
					<tr><th><c:choose><c:when test="${programmer.department == 0}">JAVA</c:when>
									  <c:when test="${programmer.department == 1}">Android</c:when>
									  <c:when test="${programmer.department == 2}">IOS</c:when>
									  <c:when test="${programmer.department == 3}">PHP</c:when></c:choose></th>
					<th>${programmer.programmerName}</th>
					<th><c:choose><c:when test="${programmer.programmerSex == 0}">男</c:when>
								  <c:when test="${programmer.programmerSex == 1}">女</c:when></c:choose></th>
					<th>${programmer.loginName}</th>
					<th>${programmer.userMobile}</th>
					<th>${programmer.programmerQq}</th>
					<th><c:choose><c:when test="${programmer.programmerStatus == 0}">空闲</c:when>
								  <c:when test="${programmer.programmerStatus == 1}">项目中</c:when></c:choose></th>
